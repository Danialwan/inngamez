 ...
 <div id="editImageModal" class="modal">
     ...
     <div class="modal-content">
         ...
         <form class="mt-5 flex flex-col items-center w-full" method="POST" action="{{ '/game/'.$game->id.'/updateImage' }}" enctype="multipart/form-data">
             @csrf
             @method('PUT')
             <b class="title text-center flex justify-self-center">Update game Image:</b>
             <div class="flex flex-col items-start w-full">
                 <label for="contact" class="block text-sm font-medium leading-6 text-gray-900">game Image</label>
                 <div class="oldImage rounded-lg" style="background-image: url('{{ asset('images/gameImage/'.$game->image) }}')"></div>
 ...
 <div id="editTitleModal" class="modal">
     ...
     <div class="modal-content">
         ...
         <form class="mt-5 flex flex-col items-center w-full" method="POST" action="{{ '/game/'.$game->id.'/updateTitle' }}">
             @csrf
             @method('PUT')
             <b class="title text-center flex justify-self-center">Update Game Title:</b>
             <div class="flex flex-col items-start w-full">
                 <label for="gameTitle" class="block text-sm font-medium leading-6 text-gray-900">
                     Title :</label>
                 <input type="text" name="gameTitle" id="gameTitle" autocomplete="given-name"
 ...
 <div id="editBodyModal" class="modal">
     ...
     <div class="modal-content">
         ...
         <form class="mt-5 flex flex-col items-center w-full" method="POST" action="{{ '/game/'.$game->id.'/updateBody' }}">
             @csrf
             @method('PUT')
             <b class="title text-center flex justify-self-center">Update game description:</b>
             <div class="flex flex-col items-start w-full">
                 <label for="gameDescription" class="block text-sm font-medium leading-6 text-black">Deskripsi Game: </label>
                 <div class="mt-2 w-full">
                     <textarea id="gameDescription" name="gameDescription" rows="10"
                         class="block w-full rounded-md border-0 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2">{{$game->description}}</textarea>
                 </div>
                 <div class="flex flex-col w-full justify-between mt-4 xl:flex-row">
                     <p style="font-size: 1rem; color: red">Pastikan kembali deskripsi game yang anda masukan telah
                         sesuai!</p>
                     <button class="btn ModalBtn rounded-lg" type="submit">Save</button>
                 </div>
             </div>
